...
;(function (window,unit) {
    ...
    Food.prototype = {
        ...
        init: function () {
            ...
            self.render();
            self.map.element.appendChild(div);

        },
        render: function () {
            self.x = unit.getRandom(self.map.element.offsetWidth / self.width, 0) * self.width;
            self.y = unit.getRandom(self.map.element.offsetHeight / self.height, 0) * self.height;
