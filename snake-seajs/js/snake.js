    ...
    Snake.prototype = {
        ...
        init: function () {
            self.body.forEach(function (item) {               
                ...
                self.scene.element.appendChild(item.element);
            })
        },
        move: function (food) {            
            var timer = window.setInterval(function () {
                ...
                var scene = self.scene.element;
                //碰壁检测
                if (self.direction === 'left' && (head.x - 1) * self.width < 0) {
                    alert('GAME OVER');
                    return window.clearInterval(timer);//不再往下执行
                } else if (self.direction === 'right' && (head.x + 1) * self.width >= scene.offsetWidth) {
                    alert('GAME OVER');
                    return window.clearInterval(timer);
                } else if (self.direction === 'up' && (head.y - 1) * self.width < 0) {
                    alert('GAME OVER');
                    return window.clearInterval(timer);
                } else if (self.direction === 'down' && (head.y + 1) * self.width >= scene.offsetHeight) {
                    alert('GAME OVER');
                    return window.clearInterval(timer);
                }
                //身体部分的每一节往前移动到上一节的位置
                ...
                if (head.x * self.width === food.x && head.y * self.height === food.y) {
                    ...
                    var element = document.createElement('div');//提取出来而不直接饮用last.element是为了scene追加和设置元素的样式
                    self.body.push({
                        x: last.x,
                        y: last.y,
                        color: last.color,
                    ...
                    self.scene.element.appendChild(element);
                    food.render();
                }
            }, 200);
